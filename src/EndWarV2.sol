...
import { IntToString } from "./libs/IntToString.sol";
 ...
 * @custom:oz-upgrades-from EndWarV1
 */
contract EndWarV2 is Initializable, OwnableUpgradeable, GameErrors, GameEvents {
  ...
  function getTerritoryUserFriendly(string calldata name) external view returns (string memory) {
    World.Territory storage territory = territories.get(name);

    bytes memory neighbors = bytes(territory.neighbors[0]);
    for (uint256 i = 1; i < territory.neighbors.length; i++) {
      neighbors = abi.encodePacked(neighbors, ", ", territory.neighbors[i]);
    }
    ...
    return
      string(
        abi.encodePacked(
          ...
          territory.name,
          " has ",
          IntToString.itoa(territory.population),
          " people and ",
          IntToString.itoa(territory.power / 1 gwei),
          " gwei power. Has borders with: ",
          neighbors
        )
      );
  }
